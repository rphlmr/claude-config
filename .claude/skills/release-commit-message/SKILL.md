...
description: Generates a Release Please-compatible multi-entry Conventional Commit message from the currently staged Git diff. Use when the user asks for a release commit message, a changelog-producing commit, or one commit representing multiple fixes, features, or breaking changes. Not for an ordinary single-line commit message.
---
