...
description: Independently verifies completed work against an approved plan and its acceptance criteria through the verifier agent. Does not implement fixes.
...
The verifier starts without this conversation, so prepare a concise,
self-contained verification brief containing:
...
Spawn exactly one `verifier` subagent with the Agent tool, as a new subagent of
that type rather than a fork of this conversation.
...
Its result may arrive later as a completion notification; wait for it before
responding with any verification outcome.
