...
description: Creates a continuation prompt that transfers established work to a fresh Claude Code session, including one in another repository.
disable-model-invocation: true
---

# Session Handoff

Write one self-contained continuation prompt addressed to a fresh Claude Code
session with no access to this conversation. Infer the relevant target work; a
handoff request transfers existing authorization and does not itself authorize
